...
// 可在init中进行初始化 监听事件 ;执行一次后关闭监听
    ...
    getPlayerDistance(){
        // console.log(this.game.playerCtrl.node.position);
    ...
    onRecycle(){
        // 回收
        // console.log("recycle",this._game.blockPool.length);
        // 回收以后能禁用update?
        // 
        this._game.blockPool.put(this.node)
    }
    //传入player y坐标进行比较, pool
    // findPlayer(y:number,pool:NodePool){
    //     if(y>this.node.position.y){
    //         // pool.put()//回收当前实例
    //         console.log('该回收了');
    //         pool.put(this.node)
    //     }
